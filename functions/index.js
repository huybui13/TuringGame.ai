...
exports.createUserDoc = functions.https.onCall(async (data, context) => {
  ...
  try {
    const bannedSnapshot = await admin
      .firestore()
      .collection('banned')
    ...
    throw new functions.https.HttpsError(
      'failed-storage',
      'Failed when storing the document in Firestore'
    );
  }
...
exports.createGameDoc = functions.https.onCall(async (data, context) => {
  ...
  try {
    ...
    throw new functions.https.HttpsError(
      'failed-storage',
      'failed when storing the game document in firestore'
    );
  }
...
exports.matchPlayersWithAI = functions.https.onCall(async (data, context) => {
  ...
  try {
    ...
    result = await db
      .collection('games')
      .where('player1', '==', data.uid)
      .where('gameStatus', '==', 'STARTED')
      .get();
    if (result.size > 0) {
      let deleteDocs = await db
        .collection('games')
        .where('player1', '==', data.uid)
        .where('gameStatus', '==', 'MATCHING')
        .get();
    ...
    await db
      ...
      .set(
        { player2: ai.docs[0].id, gameStatus: 'STARTED', isAI: true },
        { merge: true }
      );
...
exports.matchPlayersRandomly = functions.https.onCall(
  async (data, context) => {}
);

exports.matchPlayersWithPlayers = functions.https.onCall(
  async (data, context) => {}
);
...
exports.respondToPlayerGPT = functions.firestore
  ...
  .onUpdate(async (change, context) => {
    try {
      ...
      if (
        gameDoc.player2 == ai.docs[0].id &&
        gameDoc.gameStatus == 'STARTED' &&
        gameDoc.player1Questions.length > 0 &&
        gameDoc.player2Questions.length == 0
      ) {
        ...
        let aiQuestions = response.data.choices[0].text
          .trim()
          .replace('[', '')
          .replace(']', '')
          .split('@');
        ...
        let aiAnswers = response2.data.choices[0].text
          .trim()
          .replace('[', '')
          .replace(']', '')
          .split('@');
